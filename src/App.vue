              ...
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-white/40">
                  <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Temps
                      total</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Entrées
                    </th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(report, activity) in activityReports" :key="activity">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatTime(report.totalTime) }}
                    </td>
        ...
        <div class="bg-white/40 bg-clip-padding backdrop-filter backdrop-blur-xs shadow rounded-lg p-6">
          ...
          <div class="mb-6">
            <div class="flex space-x-4 mb-4">
              ...
              <button @click="addTask" class="px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700">
                Ajouter
              </button>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div v-for="column in taskColumns" :key="column.id" class="bg-white/60 p-4 rounded-lg">
              ...
              <div class="space-y-2">
                <div v-for="task in getTasksByStatus(column.id)" :key="task.id" class="bg-white p-3 rounded shadow-sm">
                  <div class="flex justify-between items-start">
                    <span class="font-medium">{{ task.title }}</span>
                    <div class="flex space-x-1">
                      <button v-if="column.id !== 'done'" @click="moveTask(task.id, getNextStatus(column.id))"
        ...
        <div class="bg-gray-400/20 bg-clip-padding backdrop-filter backdrop-blur-xs shadow rounded-lg p-6">
          ...
          <div class="mb-6">
            ...
            <div class="flex justify-end mt-2">
              <button @click="addNote" class="px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700">
                Enregistrer
              </button>
            </div>
          </div>
